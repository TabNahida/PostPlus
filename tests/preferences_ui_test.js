...
assert.equal(admin.policy.hidden,true,"Account policy stays hidden before authentication");
assert.equal(admin.policy.parent.parent.id,"preferences-dialog","Account policy belongs inside Settings");
assert.equal(admin.id("preference-spellcheck"),undefined,"Administration has no compose-only pretend preference");
admin.preferences.setAuthenticated(true);
assert.equal(admin.policy.hidden,false);
assert.equal(admin.id("preferences-title").textContent,"Settings");
admin.preferences.setAuthenticated(false);
assert.equal(admin.policy.hidden,true);
assert.equal(admin.id("preferences-title").textContent,"Display settings");
const early=fixture("admin");early.preferences.setAuthenticated(true);early.ready();
assert.equal(early.policy.hidden,false,"An early authentication response survives DOM initialization");
...
console.log("Preference dialog checks passed: generated controls, portal-specific options, live input settings, and authenticated account-policy visibility.");
